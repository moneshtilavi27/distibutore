...
<h1 style="background-color:lavender;  color: black;"class="text-center text-white">Wholesales invoice Details</h1>
			...
			<table class="fixed_header2 table table-bordered" id="table">
					  ...
					  if(isset($_POST['purch']))
					  {
					  	$strat=$_POST['stat'];$end=$_POST['end'];
							$qry="SELECT `invoice_no`.`number` AS `invoice`,`customer`.`customer_name` AS `customer`, `invoice`.`invoice_date` as `date` FROM `invoice_no`,`invoice`,`customer` WHERE `customer`.`customer_id`=`invoice`.`customer_id` AND `invoice`.`in_id`=`invoice_no`.`in_id` AND `invoice`.`invoice_date` BETWEEN '$strat' AND '$end' ORDER BY `invoice`.`invoice_date`";
						}
						else
						{
							$qry="SELECT `invoice_no`.`number` AS `invoice`,`customer`.`customer_name` AS `customer`, `invoice`.`invoice_date` as `date` FROM `invoice_no`,`invoice`,`customer` WHERE `customer`.`customer_id`=`invoice`.`customer_id` AND `invoice`.`in_id`=`invoice_no`.`in_id` ORDER BY `invoice_date`";
						}
						...
						while($out=mysqli_fetch_array($confirm))
						{
						 $sn=$sn+1; 
							$num=$out['invoice'];
							$qr="SELECT COUNT(`invoice_no`.`in_id`) AS `items`,SUM(`invoice`.`value`) AS `val` 
   						 FROM `invoice_no`,`invoice` WHERE   `invoice_no`.`in_id`=`invoice`.`in_id` and `invoice_no`.`in_id`='$num';";
					...
					?>
					      ...
					      <td><?php echo round($val,2);?></td>
					       <td><a href="wholsale_print.php?edit=<?php echo $out['invoice']; ?>"
